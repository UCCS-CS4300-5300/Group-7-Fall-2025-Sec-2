...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not users.exists():
            self.stdout.write(
                self.style.ERROR('No users found! Please run "python manage.py seed_users" first.')
            )
            return
        ...
        self.stdout.write(f'Creating {groups_per_user} groups per user for {users.count()} users...')
        ...
        with transaction.atomic():
            for user in users_list:
                for i in range(groups_per_user):
                    ...
                    num_additional_members = random.randint(1, min(3, len(users_list) - 1))
                    other_users = [u for u in users_list if u != user and user_group_count[u.id] < 2]
                    ...
                    created_count += 1
                    member_count = num_to_add + 1 if num_to_add > 0 else 1  # +1 for the creator
                    self.stdout.write(
                        self.style.SUCCESS(
