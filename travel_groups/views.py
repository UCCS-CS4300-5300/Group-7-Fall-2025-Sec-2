...
def create_group(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.info(
                request, f"Your group code is: {group.get_unique_identifier()}"
            )
...
def group_detail(request, group_id):
    ...
    accepted_trips = (
        ...
        .select_related("selected_flight", "selected_hotel", "search")
        .order_by("-created_at")
    )
    ...
    if not accepted_trips.exists():
        try:
            ...
            if consensus:
                ...
                active_option = (
                    GroupItineraryOption.objects.filter(
                        group=group, consensus=consensus, status="active"
                    )
                ...
                rejected_count = GroupItineraryOption.objects.filter(
                    group=group, consensus=consensus, status="rejected"
                ).count()
                ...
                if active_option:
                    ...
                    actual_vote_count = ItineraryVote.objects.filter(
                        option=option
                    ).count()
                    ...
                    activity_ids = (
                        json.loads(option.selected_activities)
                        if option.selected_activities
                        else []
                    )
                    ...
                    is_unanimous = (yes_votes_for_active == total_members) and (
                        votes_cast == total_members
                    )
...
def join_group(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if group.is_full:
                messages.error(
                    request, "This group is full and cannot accept new members."
                )
    ...
    try:
        ...
        if membership.role == "admin":
            ...
            if admin_count == 1:
                messages.error(
                    request,
                    "You cannot leave the group as you are the only admin. Transfer admin rights to another member first.",
                )
                return redirect("travel_groups:group_detail", group_id=group.id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            preferences, created = TravelPreference.objects.get_or_create(
                member=membership, defaults=form.cleaned_data
            )
            if not created:
...
def group_trip_management(request, group_id):
    ...
    group_itineraries = (
        GroupItinerary.objects.filter(group=group)
    ...
    for gi in group_itineraries:
        print(
            f"   - {gi.itinerary.title} (ID: {gi.itinerary.id}) added by {gi.added_by.username}"
        )
    ...
    user_itineraries = Itinerary.objects.filter(user=request.user).exclude(
    ...
    try:
        ...
        if GroupItinerary.objects.filter(group=group, itinerary=itinerary).exists():
            return JsonResponse(
                {
                    "success": False,
                    "message": "Itinerary is already linked to this group.",
                }
            )

        # Create the link
        GroupItinerary.objects.create(
            group=group, itinerary=itinerary, added_by=request.user
        )

        return JsonResponse(
            {"success": True, "message": "Itinerary added to group successfully!"}
        )
    ...
    if form.is_valid():
        try:
            ...
            with transaction.atomic():
                ...
                print(
                    f"✅ Created trip '{itinerary.title}' (ID: {itinerary.id}) for group '{group.name}'"
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            trip_pref, created = TripPreference.objects.get_or_create(
                group=group, user=request.user, defaults=form.cleaned_data
            )
...
def edit_group_trip(request, group_id, itinerary_id):
    ...
    try:
        ...
        return JsonResponse(
            {
                "success": False,
                "error": "An error occurred while editing the trip. Please try again.",
            }
        )
...
def delete_active_trip(request, group_id, option_id):
    ...
    if membership.role != "admin":
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {
                    "success": False,
                    "error": "Only group admins can delete active trips.",
                }
            )
    ...
    try:
        option = GroupItineraryOption.objects.get(
            id=option_id, group=group, status="accepted", is_winner=True
        )
        ...
        messages.success(
            request, f'Active trip "{trip_title}" has been deleted successfully.'
        )
        return redirect("travel_groups:group_detail", group_id=group.id)
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {
                    "success": False,
                    "error": "An error occurred while deleting the active trip.",
                }
            )
        messages.error(
            request,
            "An error occurred while deleting the active trip. Please try again.",
        )
        return redirect("travel_groups:group_detail", group_id=group.id)
...
def delete_group_trip(request, group_id, itinerary_id):
    ...
    try:
        group_itinerary = GroupItinerary.objects.get(
            group=group, itinerary_id=itinerary_id
        )
        ...
        messages.success(
            request, f'Trip "{trip_title}" has been removed from the group.'
        )
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {
                    "success": True,
                    "message": f'Trip "{trip_title}" removed successfully.',
                }
            )
