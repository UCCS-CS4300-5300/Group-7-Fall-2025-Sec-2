...
class NotificationTests(TestCase):
    ...
    def test_notification_error_handling(self):
        ...
        with patch('notifications.tasks.send_mail', side_effect=Exception("SMTP Error")):
            trip_pref = TripPreference.objects.create(
                group=self.group,
                user=self.user1,
                start_date=date.today() + timedelta(days=30),
    ...
    def test_notification_import_error_handling(self):
        ...
        with patch('notifications.signals.send_notification_email', side_effect=ImportError("Module not found")):
