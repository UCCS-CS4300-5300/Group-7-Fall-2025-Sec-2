...
class FlightAPIConnector(BaseAPIConnector):
    ...
    def __init__(self):
        ...
        self.api_key = os.environ.get(
            "AMADEUS_API_KEY", getattr(settings, "AMADEUS_API_KEY", None)
        )
        self.api_secret = os.environ.get(
            "AMADEUS_API_SECRET", getattr(settings, "AMADEUS_API_SECRET", None)
        )
...
class HotelAPIConnector(BaseAPIConnector):
    ...
    def search_hotels(
        ...
        return self._get_mock_hotel_data(
            destination, check_in, check_out, adults, rooms
        )

    def _get_mock_hotel_data(
        self, destination: str, check_in: str, check_out: str, adults: int, rooms: int
    ) -> List[Dict[str, Any]]:
            ...
            hotels.append(
                {
                    ...
                    * self._calculate_nights(check_in, check_out)
                    * rooms,
                    "currency": "USD",
                    "rating": rating,
                    ...
                    "cancellation_policy": random.choice(
                        ["Free cancellation", "Non-refundable", "Partially refundable"]
                    ),
...
class ActivityAPIConnector(BaseAPIConnector):
    ...
    def __init__(self):
        super().__init__()
        self.api_key = os.environ.get(
            "ACTIVITY_API_KEY", getattr(settings, "ACTIVITY_API_KEY", None)
        )
        ...
        try:
            ...
            if data.get("results") and len(data["results"]) > 0:
                ...
                for result in results:
                    ...
                    if population > 0 or feature_code in [
                        "PPL",
                        "PPLA",
                        "PPLA2",
                        "PPLC",
                ...
                print(
                    f"[WEATHER DEBUG] Geocoded to: {city_result.get('name')}, {city_result.get('country')}"
                )
                return {
                ...
                if len(parts) == 2:
                    ...
                    if region in major_cities:
                        for city in major_cities[region]:
                            ...
                            if (
                                city_data.get("results")
                                and len(city_data["results"]) > 0
                            ):
