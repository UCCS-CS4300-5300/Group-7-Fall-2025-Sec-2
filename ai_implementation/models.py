...
class TravelSearch(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="travel_searches"
    )
    ...
    activity_categories = models.TextField(
        blank=True, null=True, help_text="Comma-separated list"
    )
...
class ConsolidatedResult(models.Model):
    ...
    search = models.OneToOneField(
        TravelSearch, on_delete=models.CASCADE, related_name="consolidated_result"
    )
    ...
    # Rankings and scores
    recommended_flight_ids = models.TextField(
        blank=True,
        null=True,
        help_text="JSON list of recommended flight IDs with scores",
    )
    recommended_hotel_ids = models.TextField(
        blank=True,
        null=True,
        help_text="JSON list of recommended hotel IDs with scores",
    )
    recommended_activity_ids = models.TextField(
        blank=True,
        null=True,
    ...
    # Raw data storage
    raw_openai_response = models.TextField(
        blank=True, null=True, help_text="Full OpenAI API response"
    )
...
class FlightResult(models.Model):
    ...
    search = models.ForeignKey(
        TravelSearch, on_delete=models.CASCADE, related_name="flight_results"
    )
    ...
    ai_score = models.PositiveIntegerField(
        null=True, blank=True, help_text="AI-generated score (0-100)"
    )
    ai_reason = models.TextField(
        blank=True, null=True, help_text="AI-generated reason for recommendation"
    )

    # Destination tracking (for multi-destination searches)
    searched_destination = models.CharField(
        max_length=200,
        blank=True,
        null=True,
...
class HotelResult(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    search = models.ForeignKey(
        TravelSearch, on_delete=models.CASCADE, related_name="hotel_results"
    )
    ...
    def __str__(self):
        return f"{self.name} - ${self.total_price or 0}"

    def save(self, *args, **kwargs):
        """Ensure total_price is populated when absent."""
        if (self.total_price is None or self.total_price == 0) and self.price_per_night:
            nights = 1
            if self.search and self.search.start_date and self.search.end_date:
                nights = max((self.search.end_date - self.search.start_date).days, 1)
            nightly = Decimal(self.price_per_night)
            self.total_price = nightly * Decimal(nights)
...
class ActivityResult(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    search = models.ForeignKey(
        TravelSearch, on_delete=models.CASCADE, related_name="activity_results"
    )
...
class GroupConsensus(models.Model):
    ...
    # Consensus data
    consensus_preferences = models.TextField(
        help_text="JSON object with consensus preferences"
    )
    compromise_areas = models.TextField(
        blank=True, null=True, help_text="JSON list of compromise areas"
    )
    unanimous_preferences = models.TextField(
        blank=True, null=True, help_text="JSON list"
    )
    conflicting_preferences = models.TextField(
        blank=True, null=True, help_text="JSON list"
    )
...
class AIGeneratedItinerary(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="ai_itineraries"
    )
    search = models.ForeignKey(
        TravelSearch,
        on_delete=models.CASCADE,
    ...
    selected_activities = models.TextField(
        blank=True, null=True, help_text="JSON list of activity IDs"
    )
...
class SearchHistory(models.Model):
    ...
    search = models.ForeignKey(
        TravelSearch, on_delete=models.CASCADE, related_name="history_entries"
    )
...
class GroupItineraryOption(models.Model):
    ...
    consensus = models.ForeignKey(
        GroupConsensus, on_delete=models.CASCADE, related_name="itinerary_options"
    )
    ...
    display_order = models.PositiveIntegerField(
        default=0, help_text="Order in which options are displayed (0 = not yet shown)"
    )
    ...
    selected_flight = models.ForeignKey(
        FlightResult, on_delete=models.SET_NULL, null=True, blank=True
    )
    selected_hotel = models.ForeignKey(
        HotelResult, on_delete=models.SET_NULL, null=True, blank=True
    )
    selected_activities = models.TextField(
        blank=True, null=True, help_text="JSON list of activity IDs"
    )
    ...
    compromise_explanation = models.TextField(
        blank=True, null=True, help_text="Which preferences this option balances"
    )
...
class ItineraryVote(models.Model):
    ...
    option = models.ForeignKey(
        GroupItineraryOption, on_delete=models.CASCADE, related_name="votes"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="itinerary_votes"
    )
    group = models.ForeignKey(
        "travel_groups.TravelGroup",
        on_delete=models.CASCADE,
    ...
    comment = models.TextField(
        blank=True, null=True, help_text="Optional comment about their choice"
    )
