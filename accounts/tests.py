    ...
    def test_valid_phone_numbers(self):
        ...
        for number in valid_numbers:
            ...
            try:
                ...
                self.fail(
                    f"Phone number {number} should be valid but raised ValidationError"
                )
...
class SignupViewTest(TestCase):
    ...
    def test_signup_with_exception_raised(self):
        ...
        with patch(
            "accounts.views.UserProfile.objects.create",
...
class DashboardViewTest(TestCase):
    ...
    def test_dashboard_shows_accepted_group_trips(self):
        ...
        self.assertIsNotNone(
            our_trip, "Accepted option should be in accepted_group_trips"
        )
    ...
    def test_dashboard_weather_data_error_handling(self):
        ...
        with patch(
            "ai_implementation.api_connectors.WeatherAPIConnector"
        ) as mock_weather:
            mock_instance = MagicMock()
            mock_instance.get_weather_for_trip.side_effect = Exception(
                "Weather API error"
            )
...
class CreateItineraryViewTest(TestCase):
    ...
    def test_create_itinerary_requires_login(self):
        ...
        response = self.client.post(
            reverse("accounts:create_itinerary"),
            {
                "title": "Test Trip",
    ...
    def test_create_itinerary_success(self):
        ...
        self.assertTrue(
            Itinerary.objects.filter(user=self.user, title="Test Trip").exists()
        )
...
class DeleteItineraryViewTest(TestCase):
    ...
    def test_delete_itinerary_requires_login(self):
        ...
        response = self.client.delete(
            reverse("accounts:delete_itinerary", args=[self.itinerary.id])
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_itinerary_success(self):
        ...
        response = self.client.delete(
            reverse("accounts:delete_itinerary", args=[itinerary_id])
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertTrue(data["success"])
    ...
    def test_delete_itinerary_not_found(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        response = self.client.delete(
            reverse("accounts:delete_itinerary", args=[99999])
        )
    ...
    def test_delete_itinerary_unauthorized(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        response = self.client.delete(
            reverse("accounts:delete_itinerary", args=[other_itinerary.id])
        )
        self.assertEqual(response.status_code, 404)
        data = json.loads(response.content)
        self.assertFalse(data["success"])
    ...
    def test_delete_itinerary_exception_handling(self):
        ...
        with patch.object(
            Itinerary.objects, "get", side_effect=Exception("Database error")
        ):
            response = self.client.delete(
                reverse("accounts:delete_itinerary", args=[self.itinerary.id])
            )
